...
func NewProcessCollector() (Collector, error) {
	...
	return &ProcessCollector{
		StartTime: prometheus.NewDesc(
			...
			[]string{"process", "process_id", "creating_process_id", "process_exe_path"},
			nil,
		),
		CPUTimeTotal: prometheus.NewDesc(
			...
			[]string{"process", "process_id", "creating_process_id", "mode", "process_exe_path"},
			nil,
		),
		...
		PoolBytes: prometheus.NewDesc(
			...
			[]string{"process", "process_id", "creating_process_id", "pool", "process_exe_path"},
			nil,
		),
...
func (c *ProcessCollector) collect(ch chan<- prometheus.Metric) (*prometheus.Desc, error) {
	...
	var dstPath []Win32_Process
	qPath := wmi.CreateQuery(&dstPath, c.queryWhereClause)
	if err := wmi.Query(qPath, &dstPath); err != nil {
		return nil, err
	}
	...
	for _, process := range dst {
		...
		// Get path to the executable file of the process.
		var processExePath string

		for _, v := range dstPath {
			if v.ProcessId == process.IDProcess {
				processExePath = v.ExecutablePath
				break
			}
		}
